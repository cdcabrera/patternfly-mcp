...
export const startServer = async ({
  ...
  if (loggingArgs.protocol) {
    try {
      ...
      await mcpClient.setLoggingLevel((loggingArgs.level || 'debug') as LoggingLevel);
    } catch {
      // If SDK shape changes, avoid failing tests — stderr logs still provide signal.
      // And the server may not advertise logging.
    }
  }
