...
- **Style Preference (Nullish Coalescing)**: Avoid the nullish coalescing operator (`??`) unless explicitly justified. Prefer logical OR (`||`) or explicit `undefined`/`null` checks for consistency with established project style.
...
Expensive operations (network, I/O, schema processing) should be memoized by assigning a `.memo` property to the function.
...
const handler = async (args: unknown) => {
  if (!isPlainObject(args)) throw new McpError(ErrorCode.InvalidParams, 'Object required');
  const { name } = args;
