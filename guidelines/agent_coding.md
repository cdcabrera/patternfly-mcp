...
The codebase uses a custom memoization system for performance optimization. Functions are memoized by assigning a `.memo` property, specifically for ease in testing.
...
- Use custom `keyHash` for stable hashing when arguments are complex objects, or when exposed to external users.
...
**Suggestive Failure:**
When a not found resource is exposed to external users and agents, suggest alternatives:
```typescript
...
Extensive use of type guards for runtime type checking and TypeScript narrowing. Do not solely rely on TypeScript this is a compiled app!
...
Options and session objects are frozen by design to prevent accidental mutations.
...
Use `timeoutFunction` for async operations that should have timeouts and potentially lack response errors.
...
- **`@param`**: Document all parameters with descriptions, types are optional
- **`@returns`**: Document return what the function returns, types are optional
...
- **`@alias`**: Generally used for providing stable aliased typings exposed to consumers, minimal descriptions only
...
- Use minimal JSDoc: brief description, key parameters, return description
- Focus on "why" not "what" when the code is self-explanatory
- Add `@note` for non-obvious implementation details and future improvements

### Best Practices

- Keep descriptions concise but informative
- Use TypeScript types in JSDoc ONLY when they add clarity, OR typings differ from implementation 
