 ...
 */
// Global MCP client instance (created per audit run)
let mcpClient = null;
...
async function getMcpClient(config) {
  if (!mcpClient) {
    mcpClient = await createMcpClient(config.mcp.url, {
      timeout: config.mcp?.timeout || 10000
    });
  }
  return mcpClient;
      ...
      if (question.category === 'tooling') {
        try {
          ...
          if (question.id === 'pf-mcp-1' || question.prompt.includes('tools are available') || question.prompt.includes('resources are available')) {
            ...
            try {
              resourcesList = await client.listResources();
            } catch (resourceError) {
              console.warn(`   ⚠️  resources/list failed: ${resourceError.message}`);
            }
            ...
            if (tools.length > 0) {
              ...
              toolDescriptions = `Available PatternFly MCP tools:\n${toolDescs}`;
            } else {
              toolDescriptions = `No tools were returned from the PatternFly MCP server.`;
            }

            // Build resources description
            let resourceDescriptions = '';
            if (resources.length > 0) {
              const resourceDescs = resources.map(r => {
              ...
              resourceDescriptions = `\n\nAvailable PatternFly MCP resources:\n${resourceDescs}`;
            } else {
              resourceDescriptions = `\n\nNo resources were returned from the PatternFly MCP server.`;
            }

            toolResults = toolDescriptions + resourceDescriptions;
            toolCalls.push({
              tool: 'tools/list',
              args: {},
              timestamp: Date.now()
            ...
            const toolResult = await client.callTool('usePatternFlyDocs', {
            ...
            const toolResult = await client.callTool('componentSchemas', {
