...
const patternFlyContextCollections = (
  { collections = COLLECTIONS }: { collections?: Collection[] } = {}
): { collectionNameIndex: Map<string, Collection & { id: string; uri: string }>, collectionIdIndex: Map<string, Collection & { id: string; uri: string }> } => {
  const collectionIdIndex = new Map<string, Collection & { id: string; uri: string }>();
  const collectionNameIndex = new Map<string, Collection & { id: string; uri: string }>();

  collections.forEach(collection => {
    const collectionName = collection.name.toLowerCase();
    const collectionId = generateHash(`collection:${collectionName}`).toLowerCase();
    const collectionUri = `patternfly://collections/${collectionId}`;
    const collectionRecord = { ...collection, id: collectionId, uri: collectionUri };
