...
function validateToolInputStringLength(
  ...
  validateToolInput(
    ...
    new McpError(ErrorCode.InvalidParams, description || `"${inputDisplayName || 'Input'}" must be a string with length between "${min - 1}" and "${max + 1}" characters`)
  );
}
...
function validateToolInputStringArrayEntryLength(
  ...
  validateToolInput(
    ...
    // (vInput: unknown[]) => Array.isArray(vInput) && !vInput.every(entry => typeof entry === 'string' && entry.trim().length >= min && entry.trim().length <= max),
    new McpError(ErrorCode.InvalidParams, description || `"${inputDisplayName || 'Input'}" array must contain strings with length between "${min - 1}" and "${max + 1}" characters`)
  );
}
