...
type AssertCodeErrorFactory = ErrorCode | AssertErrorFactory;
 ...
 * @param {AssertCodeErrorFactory} [codeOrError] - Thrown error code when validation fails OR
...
const mcpAssert = (
  ...
  codeOrError: AssertCodeErrorFactory = ErrorCode.InvalidParams
) => {
  try {
...
function assertInput(
  condition: unknown,
  message: string | (() => string),
  codeOrError?: AssertCodeErrorFactory
): asserts condition {
...
function assertInputString(
  ...
  }: { inputDisplayName?: string; message?: string; codeOrError?: AssertCodeErrorFactory } = {}
...
function assertInputStringLength(
  ...
  }: { max: number; min: number; inputDisplayName?: string; message?: string; codeOrError?: AssertCodeErrorFactory }
): asserts input is string {
...
function assertInputUrlWhiteListed(
  ...
  {
    allowedProtocols = DEFAULT_OPTIONS.patternflyOptions.urlWhitelistProtocols, inputDisplayName, message, urlDisplayMaxLength = 50, codeOrError
  }: {
    allowedProtocols?: string[]; inputDisplayName?: string; message?: string; urlDisplayMaxLength?: number; codeOrError?: AssertCodeErrorFactory
  } = {}
