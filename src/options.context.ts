 ...
 *
 * Avoids cloning `toolModules`. `toolModules` are normalized and loaded subsequently.
 *
...
const setOptions = (options?: DefaultOptionsOverrides): GlobalOptions => {
  ...
  const originalToolModules = options?.toolModules && Array.isArray(options.toolModules) ? options.toolModules : [];
  const merged: GlobalOptions = mergeObjects(DEFAULT_OPTIONS, { ...options, toolModules: [] }, { allowNullValues: false, allowUndefinedValues: false });
  const cloned = structuredClone(merged);
  const restoreOriginalToolModules: GlobalOptions = { ...cloned, toolModules: originalToolModules };
...
const runWithOptions = async <TReturn>(
  ...
  const cloned = structuredClone({ ...(options as any), toolModules: [] });
  const restoreOriginalToolModules: GlobalOptions = { ...cloned, toolModules: originalToolModules };
  const frozen = freezeObject(restoreOriginalToolModules);
