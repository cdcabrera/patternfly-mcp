 ...
 */
const isCreatorsArray = (value: unknown): value is CollectionCreator[] =>
 ...
 */
const isRealizedTuple = (value: unknown): value is CollectionSource =>
  Array.isArray(value) &&
  value.length >= 2 &&
 ...
 */
const wrapCachedTuple = (cached: CollectionSource): CollectionCreator => {
  const wrapped: CollectionCreator = () => cached;

  return wrapped as CollectionCreator;
};
...
const resolveCreators = (
  ...
  for (const candidate of candidates) {
    ...
    if (isCreatorsArray(candidate)) {
      ...
      return candidate as CollectionCreator[];
    }
