...
/**
 * Type guard to check if data has metadata structure
 */
function hasMetadata(data: DocsIndexWithMetadata | DocsIndex): data is DocsIndexWithMetadata {
  return 'documents' in data && typeof data.documents === 'object';
}

/**
 * The documentation index loaded from JSON
 * Handles both old format (direct index) and new format (with metadata)
 */
const loadedData = docsIndexJson as DocsIndexWithMetadata | DocsIndex;
const docsIndex: DocsIndex = hasMetadata(loadedData) ? loadedData.documents : loadedData;
...
export const getComponentUrls = (componentName: string, version: string = '6'): string[] => {
  ...
  if (versionDocs.design) {
    urls.push(versionDocs.design);
  }
  if (versionDocs.accessibility) {
    urls.push(versionDocs.accessibility);
  }
  if (versionDocs.examples) {
    urls.push(versionDocs.examples);
  }
